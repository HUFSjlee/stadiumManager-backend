...
### 1) 예약 동시성 이슈 해결 (Redis Redisson Lock + 테스트로 검증)
- **문제**: 다수 사용자가 동시에 예약 시 최대 인원 초과/정합성 깨질 수 있음
- **해결**: **Redisson Lock** 적용으로 동일 자원(구장 예약)에 대한 동시 접근 제어
- **검증**: **30명 동시 요청 테스트**로 확인  
  - Lock 조건 / 대기시간(**wait time**) / 점유시간(**lease time**) 조정 후 정상 동작

---

### 2) JPA N+1 문제 개선 (Lazy + Fetch Join)
- **문제**: 연관 엔티티 조회 시 **N+1 쿼리**로 성능 저하 가능
- **해결**
  - 기본 로딩 전략 **EAGER → LAZY** 변경
  - 필요한 조회는 **Fetch Join**으로 한 번에 조회
- **결과**: 불필요한 쿼리 반복을 줄여 조회 성능을 안정화

---

### 3) 예외 처리 공통화 (GlobalExceptionHandler)
- **문제**: 예외 처리 방식이 API마다 달라지면 응답 포맷이 깨지고 유지보수가 어려움
- **해결**: `@RestControllerAdvice` + `@ExceptionHandler`로 공통 처리, Custom Exception 도입
- **결과**: 예외 상황에서도 **일관된 응답** 제공

---

### 4) 응답(Response) 포맷 통일 (BaseResponse)
- **문제**: API별 응답 구조가 다르면 클라이언트 처리 비용이 커짐
- **해결**: `BaseResponse`로 공통 포맷 통합 (`code / msg / result / timestamp`)
- **결과**: 응답 규격이 고정되어 클라이언트/테스트 코드가 단순해짐

---

### 5) 중복 예약 방지 (JPA Query Method)
- **문제**: 멤버가 동일 구장을 중복 예약하는 케이스 방지 필요
- **해결**: `existsByMemberIdAndReservableStadiumId`로 사전 체크
- **결과**: 중복 예약 요청을 빠르게 차단

---

### 6) 외부 API 연동 (카카오 로컬 API)
- **목적**: 주소 기반 경기장 위치 검색 기능 구현
- **해결**: 외부 응답은 필요한 값만 **DTO**로 분리해 내부 도메인으로 전달
- **결과**: 외부 스펙 변경 영향 범위를 줄이고, 내부 모델을 단순화

---

### (선택) Swagger 오류 이슈 (진행 중)
- Redis 의존성 추가 이후 Swagger 오류 로그 발생
- 동시성 처리 기능을 우선 확보했고, Swagger는 필요 시점에 원인 분석/해결 예정
